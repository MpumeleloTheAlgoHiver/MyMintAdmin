  ...
  if (req.url.startsWith('/api/team') && ['GET','POST','PUT','DELETE'].includes(req.method)) {
    ...
    (async () => {
      try {
        ...
        if (!callerId) { sendJson(res, 401, { error: 'Invalid session' }); return; }

        /* Verify caller exists in admin_team as admin */
        const callerRows = await fetchSupabaseJson(
          `/rest/v1/admin_team?user_id=eq.${callerId}&select=role`,
          null, true
        );
        const callerRole = callerRows?.[0]?.role;
        if (callerRole !== 'admin') { sendJson(res, 403, { error: 'Admin access required' }); return; }
        ...
        if (req.method === 'POST' && req.url === '/api/team/invite') {
          const body = await readJsonBody(req);
          const { email, full_name, role, page_permissions } = body || {};
          if (!email || !role) { sendJson(res, 400, { error: 'email and role are required' }); return; }
          if (!['admin','staff'].includes(role)) { sendJson(res, 400, { error: 'Invalid role' }); return; }

          /* Send Supabase magic-link invite */
          const inviteRes = await new Promise((resolve, reject) => {
            const payload = JSON.stringify({ email, data: { full_name: full_name || '' } });
          ...
          if (inviteRes.status >= 400) {
            sendJson(res, 400, { error: inviteRes.body?.msg || inviteRes.body?.message || 'Invite failed' });
            return;
          }

          const invitedUserId = inviteRes.body?.id || null;

          /* Insert into admin_team (email is unique so conflict = already invited) */
          await mutateSupabaseJson('/rest/v1/admin_team?on_conflict=email', {
            email,
        ...
        /* PUT /api/team/:id — update role / page_permissions */
        const putMatch = req.url.match(/^\/api\/team\/([^/?]+)$/);
