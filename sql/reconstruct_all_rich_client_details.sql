...
begin
  ...
  loop
    ...
    if v_kyc is null and v_idmn is null then v_pack := v_pack-'experian'; end if;

    v_first := coalesce(nullif(v_profile->>'first_name',''),nullif(v_info->>'firstName',''),jsonb_path_query_first(coalesce(v_idmn,v_kyc),'$.**.firstName')#>>'{}',jsonb_path_query_first(coalesce(v_idmn,v_kyc),'$.**.forename')#>>'{}');
    ...
    values(r.id,
      v_pack || jsonb_build_object(
        ...
        'experian',jsonb_strip_nulls(jsonb_build_object(
          'kyc',v_kyc,'idmn',v_idmn,'source','Experian verified onboarding data','reconstructed_at',now()
        ))) else '{}'::jsonb end,
      now())
