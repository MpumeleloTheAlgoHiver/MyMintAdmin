...
// ── Archive (merged from sumsub/archive.js) ───────────────────────────────────
...
const splitName = (fileName, index) => {
  ...
  if (dotIndex < 0) return { name: safe, ext: '' };
  return { name: safe.slice(0, dotIndex), ext: safe.slice(dotIndex) };
};

const supabaseStorageRequest = async (path, options = {}) => {
  const supabaseUrl = process.env.SUPABASE_URL;
  const supabaseServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !supabaseServiceRoleKey) throw new Error('Supabase server credentials are not configured');
...
const ensureArchiveBucket = async () => {
  ...
  const createBucket = await supabaseStorageRequest('/storage/v1/bucket', {
    ...
    body: JSON.stringify({ id: ARCHIVE_BUCKET, name: ARCHIVE_BUCKET, public: false, file_size_limit: 15728640 })
  });
...
const fetchSumsubJson = async (pathWithQuery) => {
  ...
  if (!headers) throw new Error('Sumsub credentials are not configured');
  const response = await fetch(`https://api.sumsub.com${pathWithQuery}`, { method: 'GET', headers });
  let payload = null;
  try { payload = await response.json(); } catch { payload = null; }
  if (!response.ok) {
...
const fetchSumsubBinary = async (pathWithQuery) => {
  ...
  if (!headers) throw new Error('Sumsub credentials are not configured');
  const response = await fetch(`https://api.sumsub.com${pathWithQuery}`, { method: 'GET', headers });
  if (!response.ok) throw new Error(`Sumsub binary request failed (${response.status})`);
...
const createSignedUrl = async (storagePath, expiresInSeconds = 3600) => {
  ...
  if (!signResponse.response.ok) return null;
...
const archiveCompletedUser = async ({ profileId, externalUserId }) => {
  ...
  if (!isApplicantCompleted(applicant)) {
    return { skipped: true, reason: 'Applicant not completed yet', archivedCount: 0, applicantId: applicant?.id || applicant?.applicantId || applicant?.applicant_id || null };
  }
  const applicantId = applicant?.id || applicant?.applicantId || applicant?.applicant_id;
  if (!applicantId) throw new Error('No applicantId found in Sumsub response');
  const inspectionId = applicant?.inspectionId || applicant?.review?.inspectionId;
  if (!inspectionId) return { skipped: true, reason: 'No inspectionId in completed applicant', archivedCount: 0, applicantId };
  ...
  if (!items.length) return { skipped: false, reason: 'No metadata resources found', archivedCount: 0, applicantId };
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    const storagePath = [String(profileId), String(applicantId), String(inspectionId), `${imageId}-${split.name}${split.ext}`].join('/');
    await uploadArchivedObject({ storagePath, mimeType, binary: buffer });
    upsertRows.push({
  ...
  return { skipped: false, archivedCount, applicantId, inspectionId };
};
...
const handleArchive = async (req, res) => {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (!profileId || !externalUserId) return sendJson(res, 400, { error: 'profileId and externalUserId are required' });
    ...
    if (method === 'GET') {
      ...
      for (const row of (Array.isArray(rows) ? rows : [])) {
        ...
        enrichedRows.push({ ...row, signed_url: signedUrl });
      }
      return sendJson(res, 200, { ok: true, items: enrichedRows });
    }
    res.setHeader('Allow', 'GET, POST');
    return sendJson(res, 405, { error: 'Method not allowed' });
  } catch (error) {
    return sendJson(res, 500, { error: 'Could not archive Sumsub resources', details: error?.message || 'Unknown error' });
  }
};
...
module.exports = (req, res) => {
  const url = req.url || '';
  if (url.includes('/sumsub/archive') || req.query?._route === 'archive') return handleArchive(req, res);
